...
static uint8 TS_I2C_slReadBuf[TS_I2C_I2C_BTLDR_SIZEOF_READ_BUFFER];

/* Reads from this buffer */
static uint8 TS_I2C_slWriteBuf[TS_I2C_I2C_BTLDR_SIZEOF_WRITE_BUFFER];

/* Flag to release buffer to be read */
static uint32 TS_I2C_applyBuffer;
...
static void TS_I2C_I2CResposeInsert(void);


/*******************************************************************************
* Function Name: TS_I2C_I2CCyBtldrCommStart
********************************************************************************
*
* Summary:
...
{
    TS_I2C_I2CSlaveInitWriteBuf(TS_I2C_slWriteBuf, TS_I2C_I2C_BTLDR_SIZEOF_WRITE_BUFFER);
    TS_I2C_I2CSlaveInitReadBuf (TS_I2C_slReadBuf, 0u);

    TS_I2C_SetCustomInterruptHandler(&TS_I2C_I2CResposeInsert);
    TS_I2C_applyBuffer = 0u;
...
/*******************************************************************************
* Function Name: TS_I2C_I2CCyBtldrCommStop
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: TS_I2C_I2CCyBtldrCommReset
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: TS_I2C_I2CCyBtldrCommRead
********************************************************************************
*
* Summary:
...
*
* Global variables:
*  TS_I2C_slWriteBuf - used to store received command.
*  TS_I2C_slWrBufIndex - used to store current index within slave
*  write buffer.
*
*******************************************************************************/
cystatus TS_I2C_I2CCyBtldrCommRead(uint8 pData[], uint16 size, uint16 * count, uint8 timeOut)
{
    ...
    if ((NULL != pData) && (size > 0u))
    {
        ...
        while (0u != timeoutMs)
        {
            ...
            if (0u != (TS_I2C_I2C_SSTAT_WR_CMPLT & TS_I2C_slStatus))
            {
                /* Copy number of written bytes */
                *count = (uint16) TS_I2C_slWrBufIndex;
                ...
                (void) memcpy((void *) pData, (const void *) TS_I2C_slWriteBuf,
                                              TS_I2C_BYTES_TO_COPY(*count, size));

                status = CYRET_SUCCESS;
                break;
            }

            CyDelay(TS_I2C_WAIT_1_MS);
            --timeoutMs;
        }
    }
...
/*******************************************************************************
* Function Name: TS_I2C_I2CCyBtldrCommWrite
********************************************************************************
*
* Summary:
...
*
* Global variables:
*  TS_I2C_slReadBuf - used to store response.
*
*******************************************************************************/
cystatus TS_I2C_I2CCyBtldrCommWrite(const uint8 pData[], uint16 size, uint16 * count, uint8 timeOut)
{
    ...
    if ((NULL != pData) && (size > 0u))
    {
        ...
        *count = size;
        (void) memcpy((void *) TS_I2C_slReadBuf, (const void *) pData, (uint32) size);

        /* Read buffer is ready to be released to host */
        TS_I2C_applyBuffer = (uint32) size;
...
/*******************************************************************************
* Function Name: TS_I2C_I2CResposeInsert
********************************************************************************
*
* Summary:
...
*
* Global variables:
*  TS_I2C_slRdBufSize - used to store slave read buffer size.
*  TS_I2C_slRdBufIndex - used to store the current index within the
*  slave read buffer.
*  TS_I2C_applyBuffer - the flag to release the buffer with
*  a response to read.
*
*******************************************************************************/
static void TS_I2C_I2CResposeInsert(void)
{
    if (TS_I2C_CHECK_INTR_SLAVE_MASKED(TS_I2C_INTR_SLAVE_I2C_ADDR_MATCH))
    {
        if (TS_I2C_CHECK_I2C_STATUS(TS_I2C_I2C_STATUS_S_READ))
        {
            ...
            if (0u != TS_I2C_applyBuffer)
            {
                TS_I2C_slRdBufSize  = TS_I2C_applyBuffer;
                TS_I2C_slRdBufIndex = 0u;
                TS_I2C_applyBuffer  = 0u;
            }
        }
        else
        {
            ...
            if (TS_I2C_slRdBufIndex != TS_I2C_slRdBufSize)
            {
                TS_I2C_slRdBufIndex = TS_I2C_slRdBufSize;
            }
        }
    }
}
